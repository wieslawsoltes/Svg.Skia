...
| Text | Partial | Supports `<text>`, `<tspan>`, `<tref>`, `<textPath>`, focused `altGlyph` substitution, x/y/dx/dy/rotate positioning, `textLength`, `lengthAdjust`, bidi direction, writing mode, text decoration, JavaScript text selection extents/static highlights, and fill/stroke text painting. Browser-complete baseline, vertical layout, textPath, shaping, glyph fallback, and exact browser selection UI behavior remain partial. |
| SVG 1.1 altGlyph text | Partial | Focused `altGlyph`, `altGlyphDef`, `altGlyphItem`, and `glyphRef` SVG-font substitution is implemented for references resolving to one SVG font entry, including empty-content `altGlyph` rows. Exact browser/font raster identity for W3C `text-altglyph-*` remains thresholded parity work. |
