---
title: "SvgML Inline SVG"
---

# SvgML Inline SVG

`SvgML` is the Avalonia XAML-first path in this repository. It turns an inline tree of SVG-like controls into SVG markup, then feeds that markup through the shared `Svg.Skia` renderer.
...
The same approach works for filter elements such as `feTurbulence`, which makes `SvgML` a good fit for small procedural effects or animated UI accents.
...
## When to use SvgML versus SvgSource

- Choose `SvgML` when the visual is authored inline and should be styleable as part of the Avalonia view.
- Choose `Avalonia.Svg.Skia` or `Avalonia.Svg` when the source is already an external SVG asset or a reusable `SvgSource`.

The repository sample at `samples/SvgML.Demo` shows both the reusable-resource pattern and selector-driven animation.
