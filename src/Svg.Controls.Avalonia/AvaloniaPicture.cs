            ...
            case DrawTextCanvasCommand drawTextCanvasCommand:
                {
                    if (drawTextCanvasCommand.Paint is { })
                    {
                        ...
                        var typeface = paint.Typeface?.ToTypeface() ?? AM.Typeface.Default;
                        ...
                        var bounds = new SKRect(0f, -ascent, width, descent);

                        var (brush, _) = paint.ToBrushAndPen(bounds);
                        var formattedText = paint.ToFormattedText(text, brush);

                        var x = drawTextCanvasCommand.X;
                        var y = drawTextCanvasCommand.Y;
                        var origin = new A.Point(x, y - paint.TextSize);
                        commands.Add(new TextDrawCommand(origin, formattedText));
                    }
                    break;
                }
