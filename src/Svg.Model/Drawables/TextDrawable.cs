...
namespace Svg.Model.Drawables
{
    public sealed class TextDrawable : DrawableBase
    {
        ...
        internal void BeginDraw(SvgTextBase svgTextBase, Canvas skCanvas, Rect skBounds, Attributes ignoreAttributes, CompositeDisposable disposable, out MaskDrawable? maskDrawable, out Paint? maskDstIn, out Paint? skPaintOpacity, out Paint? skPaintFilter)
        {
            ...
            if (enableOpacity)
            {
                skPaintOpacity = SvgExtensions.GetOpacityPaint(svgTextBase, disposable);
            ...
            if (enableFilter)
            {
                skPaintFilter = SvgExtensions.GetFilterPaint(svgTextBase, skBounds, this, disposable, out var isValid);
        ...
        internal void DrawTextString(SvgTextBase svgTextBase, string text, float x, float y, Rect skOwnerBounds, Attributes ignoreAttributes, Canvas skCanvas, DrawableBase? until)
        {
            ...
            {
                var skPaint = SvgExtensions.GetFillPaint(svgTextBase, skBounds, ignoreAttributes, Disposable);
                if (skPaint != null)
                {
                    SvgExtensions.SetPaintText(svgTextBase, skBounds, skPaint, Disposable);
#if USE_TEXT_SHAPER
            ...
            {
                var skPaint = SvgExtensions.GetStrokePaint(svgTextBase, skBounds, ignoreAttributes, Disposable);
                if (skPaint != null)
                {
                    SvgExtensions.SetPaintText(svgTextBase, skBounds, skPaint, Disposable);
#if USE_TEXT_SHAPER
        ...
        internal void DrawTextPath(SvgTextPath svgTextPath, float currentX, float currentY, Rect skOwnerBounds, Attributes ignoreAttributes, Canvas skCanvas, DrawableBase? until)
        {
            ...
            if (isValidFill || isValidStroke)
            {
                if (!string.IsNullOrEmpty(svgTextPath.Text))
                {
                    ...
                    {
                        var skPaint = SvgExtensions.GetFillPaint(svgTextPath, skBounds, ignoreAttributes, Disposable);
                        if (skPaint != null)
                        {
                            SvgExtensions.SetPaintText(svgTextPath, skBounds, skPaint, Disposable);
                    ...
                    {
                        var skPaint = SvgExtensions.GetStrokePaint(svgTextPath, skBounds, ignoreAttributes, Disposable);
                        if (skPaint != null)
                        {
                            SvgExtensions.SetPaintText(svgTextPath, skBounds, skPaint, Disposable);
                            skCanvas.DrawTextOnPath(text, skPath, hOffset, vOffset, skPaint);
