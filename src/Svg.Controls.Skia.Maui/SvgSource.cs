...
public sealed class SvgSource : IDisposable
{
    ...
    public static Uri NormalizePath(string path, Uri? baseUri = null)
    {
        ...
        var logicalName = path.TrimStart('/');
        return new Uri($"maui-app:///{Uri.EscapeDataString(logicalName).Replace("%2F", "/", StringComparison.Ordinal)}", UriKind.Absolute);
    }
    ...
    private static SKPicture? Load(SvgSource source, string? path, SvgParameters? parameters)
    {
        ...
        return ReplaceLoadedState(
            ...
            originalPath: path,
            parameters: parameters,
            originalBaseUri: new Uri(path, UriKind.RelativeOrAbsolute))
            ? picture
            : null;
    }
    ...
    private static async Task<Stream?> OpenStreamAsync(Uri uri, CancellationToken cancellationToken)
    {
        ...
        if (uri.Scheme == "maui-app")
        {
            try
            {
                var logicalName = Uri.UnescapeDataString(uri.AbsolutePath.TrimStart('/'));
                return await FileSystem.OpenAppPackageFileAsync(logicalName).WaitAsync(cancellationToken).ConfigureAwait(false);
            }
            catch (Exception e) when (e is IOException or FileNotFoundException or NotSupportedException)
            {
                Debug.WriteLine("Failed to load MAUI app package asset " + uri);
                Debug.WriteLine(e);
                return null;
            }
        }
    ...
    private static bool ReplaceLoadedState(
        SvgSource source,
        SKSvg? skSvg,
        SKPicture? picture,
        ...
        Uri? originalBaseUri)
    {
        SKPicture? previousPicture = null;
        SKSvg? previousSvg = null;
        Stream? previousStream = null;
        var disposeNewResources = false;

        lock (source.Sync)
        {
            while (source._activeRenders > 0 && !source._disposed && !source._disposePending)
            {
                Monitor.Wait(source.Sync);
            }

            if (source._disposed || source._disposePending)
            {
                disposeNewResources = true;
            }
            else
            {
                previousPicture = source._picture;
                previousSvg = source._skSvg;
                previousStream = source._originalStream;

                source._originalStream = originalStream;
                source._originalPath = originalPath;
                source._originalParameters = parameters;
                source._originalBaseUri = originalBaseUri;
                source._skSvg = skSvg;
                source._picture = picture;
            }
        }

        if (disposeNewResources)
        {
            DisposeResources(picture, skSvg, originalStream);
            return false;
        }

        DisposeResources(previousPicture, previousSvg, previousStream);
        return true;
    }
