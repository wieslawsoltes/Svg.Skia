...
public class W3CTestSuiteTests : SvgUnitTest
{
    ...
    [InlineData("coords-viewattr-04-f", 0.022, Skip = "Chrome override is checked in, but the current image-placement result on macOS is not Chrome-aligned yet.")]
    ...
    [InlineData("linking-a-05-t", 0.022, Skip = "Current macOS output does not match the checked Chrome baseline for linked transform/paint behavior.")]
    ...
    [InlineData("shapes-grammar-01-f", 0.022, Skip = "Current macOS full-suite output for negative-coordinate polyline grammar is not yet stable against the checked baseline.")]
    ...
    [InlineData("shapes-polyline-02-t", 0.022, Skip = "Current macOS polyline stroke joins/caps are not yet stable against the checked Chrome baseline.")]
    ...
    [InlineData("struct-image-19-f", 0.022, Skip = "Embedded SVG image viewport scaling is not Chrome-aligned yet on the checked macOS baseline.")]
    ...
    [InlineData("text-align-06-b", 0.022, Skip = "Current macOS text-anchor alignment output is not yet stable against the checked Chrome baseline.")]
    ...
    [InlineData("text-fonts-01-t", 0.022, Skip = "Current macOS text font fallback/composition output is not yet stable against the checked Chrome baseline.")]
    ...
    [InlineData("text-intro-01-t", 0.022, Skip = "Current macOS introductory text layout is not yet stable against the checked Chrome baseline.")]
    [InlineData("text-intro-02-b", 0.022)]
    [InlineData("text-intro-03-b", 0.022)]
    [InlineData("text-intro-04-t", 0.022, Skip = "Current macOS introductory text spacing/layout is not yet stable against the checked Chrome baseline.")]
    ...
    [InlineData("text-path-02-b", 0.022, Skip = "Current macOS textPath glyph placement diverges semantically from the checked Chrome baseline.")]
    [InlineData("text-spacing-01-b", 0.022)]
    [InlineData("text-text-01-b", 0.022, Skip = "Current macOS text layout output is not yet stable against the checked Chrome baseline.")]
