...
- `SvgML` now lives in the same repository, so inline Avalonia XAML-authored SVG trees build against the local `Svg.Skia` sources and ship from the same release pipeline.
